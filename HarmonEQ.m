...
%             'Mapping',{'pow', 2, 0.5, 100}),...
    ...
    properties (Access = private, Hidden)
        ...
        rootFilter1GainStep = 100;
        rootFilter1QDiff = 26;
        rootFilter1QTarget = 26;
        rootFilter1QSmooth = false
        rootFilter1QStep = 100;
        
        %test
        rootFilter2SmoothStatus = false; %todo: deprecated?
        rootFilter2SmoothStep = 0; %todo: deprecated?
        rootFilter2GainDiff = 0;
        rootFilter2GainTarget = 0;
        rootFilter2QDiff = 26;
        rootFilter2QTarget = 26;
        rootFilter2GainSmooth = false;
        rootFilter2GainStep = 100;
        rootFilter2QSmooth = false
        rootFilter2QStep = 100;
        ...
        thirdFilter4SmoothStatus = false;
        thirdFilter4SmoothStep = 0;
        thirdFilter4GainDiff = 0;
        thirdFilter4GainTarget = 0;
        thirdFilter4QDiff = 26;
        thirdFilter4QTarget = 26;
        
        thirdFilter6SmoothStatus = false;
        thirdFilter6SmoothStep = 0;
    ...
    methods
        ...
        function set.lowMidRegionGain(plugin,val)
            ...
            if (plugin.rootFrequency2 > plugin.lowCrossoverFreq)
                ...
                %test setUpdateRootFilter2(plugin);
            end
    ...
    methods (Access = private)
        ...
        function buildRootFilter2(plugin, fs)
            ...
            if ~plugin.rootFilter2GainSmooth && ~plugin.rootFilter2QSmooth
                ...
                disp('buildRootFilter2() thinks smoothing is not active');
            ...
            disp('buildRootFilter2() called');
            updateStateChangeStatus(plugin, true);
        end
        
        %todo: deprecated? delete?
        function buildRootFilter2_temp(plugin, fs)
        ...
        function buildThirdFilter2(plugin, fs)
            if ~plugin.thirdFilter2SmoothStatus % No smoothing necessary
                ...
                step = plugin.thirdFilter2SmoothStep;
                if (step < plugin.numberOfSmoothSteps)
                    ...
                    plugin.thirdFilter2SmoothStep = step + 1;
                    % Do not set updatethirdFilter2 to false because we want
                    % it to continue updating until we finish the smoothing
                    % operation
                    
                    plugin.thirdGain2 = gain; %store updated third gain
                    ...
                    [plugin.thirdCoeffb2, plugin.thirdCoeffa2] = peakNotchFilterCoeffs(...
                        ...
                        qFactor,...
                        gain);
                    plugin.thirdFilter2SmoothStatus = false;
                    plugin.updateThirdFilter2 = false; % No need to update further since smoothing complete
        ...
        function buildThirdFilter4(plugin, fs)
            if ~plugin.thirdFilter4SmoothStatus % No smoothing necessary
                ...
                step = plugin.thirdFilter4SmoothStep;
                if (step < plugin.numberOfSmoothSteps)
                    ...
                    plugin.thirdFilter4SmoothStep = step + 1;
                    % Do not set updateThirdFilter4 to false because we want
                    % it to continue updating until we finish the smoothing
                    % operation
                    
                    plugin.thirdGain4 = gain; %store updated third gain
                    ...
                    [plugin.thirdCoeffb4, plugin.thirdCoeffa4] = peakNotchFilterCoeffs(...
                        ...
                        qFactor,...
                        gain);
                    plugin.thirdFilter4SmoothStatus = false;
                    plugin.updateThirdFilter4 = false; % No need to update further since smoothing complete
        ...
        function buildThirdFilter6(plugin, fs)
            if ~plugin.thirdFilter6SmoothStatus % No smoothing necessary
                ...
                step = plugin.thirdFilter6SmoothStep;
                if (step < plugin.numberOfSmoothSteps)
                    ...
                    plugin.thirdFilter6SmoothStep = step + 1;
                    % Do not set updateThirdFilter6 to false because we want
                    % it to continue updating until we finish the smoothing
                    % operation
                    
                    plugin.thirdGain6 = gain; %store updated third gain
                    ...
                    [plugin.thirdCoeffb6, plugin.thirdCoeffa6] = peakNotchFilterCoeffs(...
                        ...
                        qFactor,...
                        gain);
                    plugin.thirdFilter6SmoothStatus = false;
                    plugin.updateThirdFilter6 = false; % No need to update further since smoothing complete
        ...
        %test
        %todo: revise similar functions to mimic this one
        ...
        %todo: delete this once new version is working and tested
        function updateRootFilter2Params_temp(plugin)
        ...
        function updateRootQFactor1(plugin,val)
            ...
            disp('updateRootQFactor1() called');
        end
        ...
        function updateRootQFactor2(plugin,val)
            ...
            disp('updateRootQFactor2() called');
        ...
        function updateThirdFilter2Params(plugin)
            if plugin.thirdFrequency2 < plugin.lowCrossoverFreq % third filter 2 is in low region
                if plugin.thirdFilter2Region == 1 % Already in low region
                    % Update values if smoothing is done
                    %todo: should this reset the smoothing instead?
                    if ~plugin.thirdFilter2SmoothStatus
                        plugin.thirdGain2 = plugin.lowRegionGain;
                        plugin.thirdQFactor2 = plugin.lowRegionQFactor;
                    end
                    
                else % Was in low-mid region (2)
                    plugin.thirdFilter2Region = 1; % set filter region to low (1)
                    ...
                    plugin.thirdFilter2SmoothStep = 0; % Reset the step counter for smoothing
                    plugin.thirdFilter2SmoothStatus = true; % Activate gain smoothing
                    % Updating plugin.thirdGain2 will be taken care of by
                    % buildThirdFilter2()
                    
                end
            else % Third filter 2 is in low-mid region
                if plugin.thirdFilter2Region == 2 % Already in low-mid region
                    % Update values if smoothing is done
                    if ~plugin.thirdFilter2SmoothStatus
                    ...
                    plugin.thirdFilter2Region = 2; % set filter region to low (1)
        ...
        function updateThirdFilter4Params(plugin)
            if plugin.thirdFrequency4 < plugin.lowMidCrossoverFreq % Third filter 4 is in low-mid region
                if plugin.thirdFilter4Region == 2 % Already in low-mid region (2)
                    % Update values if smoothing is done
                    %todo: should this reset the smoothing instead?
                    if ~plugin.thirdFilter4SmoothStatus
                        plugin.thirdGain4 = plugin.lowMidRegionGain;
                        plugin.thirdQFactor4 = plugin.lowMidRegionQFactor;
                    end
                    
                else % Was in mid region (3)
                    plugin.thirdFilter4Region = 2; % set filter region to low-mid (2)
            ...
            else % Third filter 4 is in mid region
                if plugin.thirdFilter4Region == 3 % Already in mid region
                    % Update values if smoothing is done
                    if ~plugin.thirdFilter4SmoothStatus
                    ...
                    plugin.thirdFilter4Region = 3; % set filter region to mid (3)
        ...
        function updateThirdFilter6Params(plugin)
            if plugin.thirdFrequency6 < plugin.midHighCrossoverFreq % Third filter 6 is in mid region
                if plugin.thirdFilter6Region == 3 % Already in mid region
                    % Update values if smoothing is done
                    %todo: should this reset the smoothing instead?
                    if ~plugin.thirdFilter6SmoothStatus
                        plugin.thirdGain6 = plugin.midRegionGain;
                        plugin.thirdQFactor6 = plugin.midRegionQFactor;
                    end
                    
                else % Was in high-mid region (4)
                    plugin.thirdFilter6Region = 3; % set filter region to low (4)
                    ...
                    plugin.thirdFilter6SmoothStep = 0; % Reset the step counter for smoothing
                    plugin.thirdFilter6SmoothStatus = true; % Activate gain smoothing
                    % Updating plugin.thirdGain6 will be taken care of by
                    % buildThirdFilter6()
                    
                end
            else % Third filter 6 is in high-mid region
                if plugin.thirdFilter6Region == 4 % Already in high-mid region
                    % Update values if smoothing is done
                    if ~plugin.thirdFilter6SmoothStatus
                    ...
                    plugin.thirdFilter6Region = 4; % set filter region to high-mid (4)
